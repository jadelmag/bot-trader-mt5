...
class CandlePatterns:
    ...
    def is_marubozu(candles, index=-1):
        ...
        df = pd.DataFrame(candles)
        ema_50 = df['close'].ewm(span=50, adjust=False).mean()
        ema_200 = df['close'].ewm(span=200, adjust=False).mean()  # EMA más lenta para confirmar tendencia
        ...
        # Calcular ADX para medir fuerza de tendencia
        ...
        # Verificar si es marubozu (cuerpo grande)
        is_marubozu_shape = candle_range > 0 and body_size / candle_range > 0.95
        if not is_marubozu_shape: return None
        
        # INVERTIR LA LÓGICA: Usar marubozu como señal de REVERSIÓN, no continuación
        
        # Marubozu alcista (vela verde) como señal de reversión bajista
        if (candle['close'] > candle['open'] and      # Vela alcista
            adx.iloc[index] > 25 and                  # Tendencia fuerte
            rsi.iloc[index] > 75 and                  # Sobrecompra extrema
            candle['close'] > ema_50.iloc[index] and  # Por encima de EMA50
            candle['close'] > ema_200.iloc[index]):   # En tendencia alcista general
            return 'short'  # Reversión bajista
        
        # Marubozu bajista (vela roja) como señal de reversión alcista
        elif (candle['open'] > candle['close'] and     # Vela bajista
            adx.iloc[index] > 25 and                 # Tendencia fuerte
            rsi.iloc[index] < 25 and                 # Sobreventa extrema
            candle['close'] < ema_50.iloc[index] and # Por debajo de EMA50
            candle['close'] < ema_200.iloc[index]):  # En tendencia bajista general
            return 'long'  # Reversión alcista
    ...
    def is_doji_reversal(candles, index=-1):
        ...
        # Calcular RSI para filtros de momentum
        delta = df['close'].diff()
        gain = (delta.where(delta > 0, 0)).ewm(alpha=1/14, adjust=False).mean()
        loss = (-delta.where(delta < 0, 0)).ewm(alpha=1/14, adjust=False).mean()
        rs = gain / loss
        ...
        # Verificar si es un Doji (sin cambios en esta parte)
        if candle_range > 0 and body_size / candle_range < 0.1:
            # Para señal LONG: 
            # - Vela anterior bajista
            # - Precio por debajo de EMA50 (tendencia bajista)
            # - RSI en zona de sobreventa (< 35)
            if (prev_candle['close'] < prev_candle['open'] and 
                candle['close'] < ema_50.iloc[index] and 
                rsi.iloc[index] < 35):
                return 'long'
                
            # Para señal SHORT:
            # - Vela anterior alcista
            # - Precio por encima de EMA50 (tendencia alcista)
