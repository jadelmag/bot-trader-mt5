...
class MomentumTooltipHandler:
    """Maneja el tooltip para el indicador Momentum"""
    
    def __init__(self, ax, canvas, period=10):
        self.ax = ax
        self.canvas = canvas
        self.period = period
        self.tooltip = None
    ...
    def update_data(self, momentum_data, timestamps, period):
    ...
    def _on_motion(self, event):
        ...
        if event.inaxes != self.ax or self.momentum_data is None:
            self._hide_tooltip()
            return
        ...
        idx = int(round(x))
        if idx < 0 or idx >= len(self.momentum_data):
            self._hide_tooltip()
            return
        
        # Verificar si está cerca de la línea
        momentum_value = self.momentum_data[idx]
        
        if np.isnan(momentum_value):
            self._hide_tooltip()
            return
        ...
        # Verificar cercanía a línea Momentum
        if not np.isnan(momentum_value) and abs(y - momentum_value) <= tolerance:
            timestamp = self.timestamps[idx]
            text = self._format_momentum_tooltip(momentum_value, timestamp)
            self._show_tooltip((x, momentum_value), text)
        else:
            self._hide_tooltip()

    def _format_momentum_tooltip(self, momentum_value, timestamp):
        """Formatea el texto del tooltip del Momentum"""
        return (
            f"Momentum({self.period})\n"
            f"{timestamp:%d %b %Y %H:%M}\n"
            f"Valor: {momentum_value:.5f}"
        )
...
class BodyMomentum(ttk.Frame):
    def __init__(self, parent, app, symbol="EURUSD", timeframe="M5", bars=300, logger=None, debug_mode_var=None, period=10, *args, **kwargs):
        ...
        # Tooltip handler
        self.tooltip_handler = MomentumTooltipHandler(self.ax, self.canvas, period=self.period)
        ...
        self._draw_placeholder("Cargando Momentum...")
    ...
    def update_momentum_data(self, candles_df):
        """Actualiza el gráfico Momentum con nuevos datos de velas"""
        if candles_df is None or candles_df.empty:
            return
        
        try:
            # Calcular Momentum usando pandas_ta
            import pandas_ta as ta
            momentum_result = ta.mom(candles_df['Close'], length=self.period)
            
            if momentum_result is None or momentum_result.empty:
                return
            
            self.momentum_data = momentum_result.values
            ...
            if self.logger:
                self.logger.error(f"Error al calcular Momentum: {e}")
    
    def _render_momentum_chart(self):
        """Renderiza el gráfico Momentum"""
        if self.momentum_data is None or len(self.momentum_data) == 0:
            return
        ...
        # Configurar ejes
        self.ax.set_ylabel('Momentum', color='#aaaaaa', fontsize=9)
        ...
        indices = np.arange(len(self.momentum_data))
        
        # Dibujar línea Momentum
        valid_momentum_mask = ~np.isnan(self.momentum_data)
        if np.any(valid_momentum_mask):
            self.momentum_line = self.ax.plot(
                indices[valid_momentum_mask],
                self.momentum_data[valid_momentum_mask],
                color='#00bcd4',  # Cyan
                linewidth=1.5,
                label=f'Momentum({self.period})',
                zorder=10
            )[0]
        ...
        if self.tooltip_handler:
            self.tooltip_handler.update_data(self.momentum_data, self.timestamps, self.period)
    ...
    def clear(self):
        """Limpia el gráfico Momentum"""
        self._draw_placeholder("Sin datos Momentum")